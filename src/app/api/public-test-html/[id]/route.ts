...
// Serves a PUBLIC test's sanitized HTML with no authentication. Only tests
// explicitly flagged `is_public = true` are served here, and the HTML is
// stripped of its answer key / explanations / evidence / download tools first
// (see sanitizePublicTestHtml). Grading happens server-side via /api/public-grade.
